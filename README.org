...
***** =programs.emacs.init.keybinds.avy.avyKeys=
The keys to use when jumping with avy.

Example usage:
#+begin_src nix-ts
programs.emacs.init.keybinds.avy.avyKeys = ["c" "r" "s" "t" "b" "f" "n" "e" "i" "a"];
#+end_src
...
***** =programs.emacs.init.completions.vertico.posframe=
Enables vertico-posframe.
...
**** =programs.emacs.init.ide.dap.enable=
Enables dap mode. Debugger pre-installation is hit or miss, since a lot of them aren't in nixpkgs.

Languages with pre-installed debuggers:
+ C
+ JS/TS
+ Java
+ Dart
...
=dap-debug= is bound to =C-x C-a=

**** =programs.emacs.init.ide.dape.enable=
Enables dape. Debugger pre-installation is hit or miss, since a lot of them aren't in nixpkgs.
...
Dape's global map is bound to =C-x C-a=
...
**** Magit
***** =programs.emacs.init.ide.magit.enable=
Enables magit support. The following keys will be bound in =project-prefix-map=:
+ v: =magit-status=
+ c: =magit-commit=
+ p: =magit-pull=
+ P: =magit-push=
...
***** =programs.emacs.init.ide.magit.forge=
Enables [[https://github.com/magit/forge][forge]] & [[https://github.com/wandersoncferreira/code-review][code-review]]. Stolen from Doom.

***** =programs.emacs.init.ide.magit.todo=
Enables [[https://github.com/alphapapa/magit-todos][magit-todos]], allowing you to see project todos in your magit-status buffer.

**** Treesit Fold
***** =programs.emacs.init.ide.treesit-fold.enable=
Enables code folding with treesitter

***** =programs.emacs.init.ide.treesit-fold.enabledModes=
A list of modes to enable =treesit-fold-mode= for

Example:
#+begin_src nix-ts
programs.emacs.init.ide.treesit-fold.enabledModes = ["bash-ts-mode" "emacs-lisp-mode"];
#+end_src

**** =programs.emacs.init.ide.citre.enable=
Enables citre. It provides a way to peek at symbol definitions, & an alternate backend to xref for when you can't use lsps. Borrowed from John Weigley's config.

Local leader keys:
+ p: peek (with =citre-peek= by default, or =citre-ace-peek=) if avy is enabled.
+ u: update the project's ctags file

Evil support:
+ You can travel up & down with M-[keybinds.evil.keys.up] & M-[keybinds.evil.keys.down]
...
If you're using apheleia, shfmt is the default formatter
...
****** =c.preferGdb=
Boolean value that installs gdb when a dap client is enabled instead of lldb
...
***** =godot.enable=
Boolean option that enables gdscript support.

It is very important that you start the godot editor before opening any gdscript/tscn files if you intend to use a language server.
+ The godot editor is preinstalled
+ Due to the odd nature of the language server, lspce & lsp-bridge are not supported

[[https://www.hristos.co/blog/godot-engine-editing-gdscript-with-emacs/][This]] is a good webpage to read for some tips.
+ gdformat is preinstalled for your convenience.
...
Due to vscode-langservers-extracted crashing recently, I have borrowed & modified the build instructions for the language server from nixpkgs to use regular nodejs instead of nodejs-slim.
...
****** =javascript.languageServer=
The language server you want to use with javascript & typescript. Can be =typscript-language-server= or =deno=.

****** =javascript.wantEslint=
Enables eslint in addition to your other js/ts language servers

Due to vscode-langservers-extracted crashing recently, I have borrowed & modified the build instructions for the language server from nixpkgs to use regular nodejs instead of nodejs-slim.

****** =javascript.wantOxlint=
Enables oxlint in addition to your other js/ts language servers

****** =javascript.wantOxfmt=
Enables oxfmt in addition to your other js/ts language servers

****** =javascript.wantBiome=
Enables biome in addition to your other js/ts language servers
...
If you're using apheleia, nixfmt is the default formatter
...
****** =org.toc.enable=
Enables [[https://github.com/snosov1/toc-org][toc-org]] for table of contents. Supports markdown if its preset is enabled.
...
***** python
Many features are borrowed from the wonderful [[https://github.com/martibosch/snakemacs][Snakemacs]]
****** =python.enable=
...
If org is enabled, you can run jupyter notebooks from an org file.

We may switch to jupyter-ascending at some point
...
****** =python.snakemake=
Adds support for snakemake files

****** =python.wantRuff=
Enables ruff in addition to your other language server.
...
***** =typescript.enable=
Boolean option to enable typescript support. Automatically enables javascript support as well, & uses its language server options
...
***** =yara.enable=
Boolean option to enable yara support.
...
**** =config.programs.emacs.init.tools.sensibleDefaults.enable=
+ Enables many sensible default settings from Emacs from Scratch & Prot's config.
  - Use short answers & recursive minibuffers are enabled.
  - User emacs directory is changed to "~/.cache/emacs"
  - All buffers are forced to obey display-buffer rules
  - Unusable M-x commands are removed from the window
+ Enables [[https://github.com/bbatsov/super-save][super-save-mode]] to automatically save files when idle. Backups are disabled.

**** =config.programs.emacs.init.tools.linNumbers.enable=
Enables line numbers. Borrowed from Emacs from Scratch.

**** =config.programs.emacs.init.tools.pdf=
Enables [[https://github.com/vedang/pdf-tools][pdf-tools]] for reading pdf files

**** dired
***** =config.programs.emacs.init.tools.dired.enable=
Enables some small dired improvements.
+ If leader keys are enabled, leader + d will open dired.
+ C-x C-j runs =dired-jump=
+ w starts wdired
+ Diredfl is used for syntax highlighting
+ Hides dotfiles by default. They can be shown again with =H=.

***** =config.programs.emacs.init.tools.dired.narrow=
Binds =N= to =dired-narrow-fuzzy=

***** =config.programs.emacs.init.tools.dired.posframe=
Binds =M-t= to =dired-posframe-mode=

***** =config.programs.emacs.init.tools.dired.async=
Runs file movement asynchronously

***** Dirvish
****** =config.programs.emacs.init.tools.dired.dirvish.enable=
Enables dirvish support. Largely stolen from doom.

****** =config.programs.emacs.init.tools.dired.dirvish.previews=
Enables previews for find-file & similar commands via dirvish-peek-mode

**** Apheleia
***** =config.programs.emacs.init.tools.apheleia.enable=
Enables apheleia for code formatting.

***** =config.programs.emacs.init.tools.apheleia.autoFormat=
Formats buffers on save.

***** =config.programs.emacs.init.tools.apheleia.modeFormatters=
Binds apheleia formatters to each major mode.

Example:
#+begin_src nix-ts
tools.apheleia.modeFormatters = {
  bash-ts-mode = "eglot";
  python-ts-mode = "(isort black)";
};
#+end_src

This option is used to set the =apheleia-formatters= variable in elisp. You should avoid altering that variable by hand.

***** =config.programs.emacs.init.tools.apheleia.formatters=
Creates definitions for apheleia formatters. You can use the macro =make-apheleia-formatter= in order to simplify making formatters in elisp.

Example:
#+begin_src nix-ts
tools.apheleia.formatters = {
  nixfmt = ["nixfmt" "-"];
  eglot = ''
...
This option is used to set the =apheleia-mode-alist= variable in elisp. You should avoid altering that variable by hand.

Eglot users get the =eglot= formatter automatically, & lsp-mode users get the =lsp= formatter automatically

**** exwm
***** =config.programs.emacs.init.tools.exwm.enable=
Enables exwm.
+ It is important that you set =exwm-randr-workspace-monitor-plist= in order to get your monitor setup right.
+ =M-x=, =C-x=, & =C-c= are all passed through to emacs buffers by default (as seen in emacs from scratch).
  - Like in emacs from scratch, any keypress can be sent directly to the exwm buffer with =C-q=
+ You will automatically be placed on workspace 0 on boot

***** =config.programs.emacs.init.tools.exwm.useGaps=
Creates some gaps around the emacs frame using [[https://github.com/firefoxsalesman/exwm-outer-gaps][Exwm Outer Gaps]].

***** =config.programs.emacs.init.tools.exwm.wantMouseWarping=
Automatically move the mouse to the center of the focused window

***** =config.programs.emacs.init.tools.exwm.titleAlterations=
Lets you set alternate titles for exwm buffers. The left hand side is the buffer's =exwm-class-name=. The right hand side is the lisp code to generate the new title

Example:
#+begin_src nix-ts
programs.emacs.init.tools.exwm.titleAlterations.qutebrowser = ''(exwm-workspace-rename-buffer (format "Qutebrowser: %s" exwm-title))'';
#+end_src

***** =config.programs.emacs.init.tools.exwm.bindings=
Keybindings for exwm. Left hand side is the key, right hand side is the command. Exported in the form ([key] . command). These items are syntax quoted for your convenience. In order to use a modifier key, you must prefix the modifier with =\\?=
Example:
#+begin_src nix-ts
programs.emacs.init.tools.exwm.bindings."?\\s-r" = "exwm-reset";
#+end_src

**** =config.programs.emacs.init.tools.goldenRatio=
Enables [[https://github.com/roman/golden-ratio.el][golden-ratio-mode]]

**** =config.programs.emacs.init.tools.noLittering.enable=
Enables no-littering. Borrowed from Emacs from Scratch
...
**** Denote
***** =config.programs.emacs.init.writing.denote.enable=
...
***** =config.programs.emacs.init.writing.denote.sequence=
Enables the denote-sequence extension.

If you have leader keys enabled, you'll gain the following keybinds:
+ =oqc=- creates a new sequence note
+ =oqf=- find a sequence note
+ =oqp=- make the current note a parent note for sequences
+ =oqr=- make the current note a child note of another note.
...
*** aesthetics
**** icons
***** =programs.emacs.init.aesthetics.icons.enable=
Enables nerd-icons in various emacs modes that are in modules supported by this framework.

***** =programs.emacs.init.aesthetics.icons.forceInstallAllTheIcons=
Forcibly installs all-the-icons, even if no package that this framework supports would take advantage of it.

***** =programs.emacs.init.aesthetics.icons.forceInstallNerdIcons=
Forcibly installs nerd-icons even if no package that this framework supports would take advantage of it.
**** =programs.emacs.init.aesthetics.dashboard.enable=
Enables the emacs-dashboard package & a basic configuration

**** =programs.emacs.init.aesthetics.declutter.enable=
Remove screen clutter such as the fringe, tooltips, the scrollbar, the toolbar, & the menubar. Borrowed from Emacs from Scratch.

*** AI
**** Copilot
***** =programs.emacs.init.ai.copilot.enable=
Enables copilot completion.

***** =programs.emacs.init.ai.copilot.keepOutOf=
Major modes to not activate copilot in

Type: list of str

Example:
#+begin_src nix-ts
programs.emacs.init.ide.copilot.keepOutOf = ["c-ts-mode" "json5-ts-mode" "json-ts-mode" "LaTeX-mode"];
#+end_src

**** =programs.emacs.init.ai.minuet.enable=
Enables completions with minuet. You'll need to set up a model to enable the completions, & you can find the documentation [[https://github.com/milanglacier/minuet-ai.el][here]].

**** Gptel
***** =programs.emacs.init.ai.gptel.enable=
A clone of doom's gptel config.

Leader keys:
+ =gp=- Open a new gptel buffer
+ =gt=- Add the current item at point (or the current region) to gptel's context
+ =gf=- Add a file to gptel's context
+ =gm=- Open gptel's configuration menu
+ =gr=- Rewrite the current region
+ =ge=- Explain the current region (via [[https://github.com/karthink/gptel-quick][gptel-quick]])
+ =go=- Limit gptel's context to the current org heading (only available if =programs.emacs.init.ide.languages.org.enable= is true.)
+ =gO=- Store gptel's config as org properties in the current buffer (only available if =programs.emacs.init.ide.languages.org.enable= is true.)

If =programs.emacs.init.ide.magit.enable= is true, [[https://github.com/ragnard/gptel-magit][gptel-magit]] will be installed.
+ Press =dx= on a magit diff section to get an explanation
+ Press =M-g= in a commit buffer to generate a message
+ Press =g= in the commit transient to create a commit with a generated message.

If =programs.emacs.init.ide.languages.org.enable= is true, [[https://github.com/jwiegley/ob-gptel][ob-gptel]] will be installed.

***** Macher
****** =programs.emacs.init.ai.gptel.macher.enable=
Enables [[https://github.com/kmontag/macher][macher]], a lightweight set of agent-like tools.

Leader keys:
+ =gMi=- Create a patch implementing whatever the user says to implement.
+ =gMr=- Revise a patch based on the user's instructions.
+ =gMd=- Discuss a piece of code with the llm.
+ =gMa=- Stop the llm's current activities

****** =programs.emacs.init.ai.gptel.macher.agent=
Enables [[https://github.com/elij/macher-agent][macher-agent]], which brings macher much closer to a traditional agent experience.

***** Introspection
Allows gptel to read emacs documentation via [[https://github.com/positron-solutions/ragmacs][ragmacs]].

***** =programs.emacs.init.ai.gptel.introspection.enable=
Provides a gptel preset called =introspect= that allows it to read emacs documentation.

***** =programs.emacs.init.ai.gptel.introspection.model=
The model to be used with the introspect preset. If left blank, your currently selected model will be used.

***** =programs.emacs.init.ai.gptel.agent.enable=
Enables [[https://github.com/karthink/gptel-agent][gptel-agent]].
