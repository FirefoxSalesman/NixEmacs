...
***** =ghookf=
The =:ghook= option from general.el, but with "-hook" appended to the end of every hook

Example usage:
#+begin_src nix
  programs.emacs.init.usePackage.envrc = {
    enable = true;
    ghook = ["('after-init 'envrc-global-mode)"];
  };
#+end_src
...
***** =gfhookf=
The =:gfhook= option from general.el, but with "-hook" appended to the end of every hook
