...
in
{
  options.programs.emacs.init.completions.helm = {
    ...
    swiperReplaceSearch = lib.mkEnableOption "Swiper replaces isearch/evil search.";
  };
  ...
  config.programs.emacs.init = lib.mkIf completions.helm.enable {
    ...
    usePackage = {
      ...
      helm = {
        ...
        config = ''(add-to-list 'completion-styles ${
          if completions.helm.fuzzy then "'flex" else "'helm"
        })'';
