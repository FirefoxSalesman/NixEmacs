...
in
{
  options.programs.emacs.init.completions.smallExtras = {
    enable = lib.mkEnableOption "Enables small completion additions, like consult & marginalia";
  ...
  config.programs.emacs.init = lib.mkIf completions.smallExtras.enable {
    ...
    usePackage = {
      ...
      consult = {
        ...
        generalOne = {
          ":nmvo" = lib.mkIf (completions.smallExtras.evilConsultLine && completions.orderless) {
      ...
      embark = lib.mkIf completions.smallExtras.embark {
        ...
        generalTwo = lib.mkIf keybinds.evil.enable {
          ...
          ":n".minibuffer-local-completion-map.":" = "'embark-act";
        };
        custom = {
