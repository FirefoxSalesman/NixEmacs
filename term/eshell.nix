...
in
{
  options.programs.emacs.init.terminals.eshell =
    lib.mkEnableOption "Enables eshell. Eat & some kind of in-buffer completion are also recommended.";

  config.programs = lib.mkIf terminals.eshell {
    bash.enable = true;
    fish.enable = true;
