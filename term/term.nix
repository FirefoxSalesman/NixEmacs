...
in
{
  options.programs.emacs.init.terminals.term =
    lib.mkEnableOption "Largely just installs multi-term. Largely stolen from doom.";
  ...
  config.programs.emacs.init.usePackage.multi-term = lib.mkIf terminals.term {
    ...
    generalOne.global-leader."t" =
      lib.mkIf config.programs.emacs.init.keybinds.leader-key.enable "'multi-term";
  };
}
