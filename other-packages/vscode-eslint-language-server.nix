...
pkgs.stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    install -Dm644 "${pkgs.vscode-extensions.dbaeumer.vscode-eslint}/share/vscode/extensions/dbaeumer.vscode-eslint/server/out/eslintServer.js" \
      "$out/lib/extensions/$server"
    makeBinaryWrapper ${lib.getExe pkgs.nodejs} "$out/bin/vscode-eslint-language-server" \
  ...
  passthru.tests.initialization =
    pkgs.runCommandLocal "vscode-langservers-extracted-initialization"
      ...
      ''
        ...
        echo "Checking eslint language server"
        response=$(request | timeout 3 "vscode-eslint-language-server" --stdio) || true
  ...
  meta = {
    ...
    description = "ESLint language server extracted from vscode";
  };
})
