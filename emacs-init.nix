...
let
        ...
        assembly =
          let
            ...
            mkGeneralLocalHelper =
              state: bs:
              let
                mkMap = n: v: mkGeneralHelper "${state}" "${n}" v;
              in
            ...
            mkGeneral =
              bs: optionals (bs != { }) ([ "(" ] ++ mapAttrsToList (n: v: ''"${n}" ${v}'') bs ++ [ ")" ]);
            mkGeneralOne =
              bs:
              let
                mkMap = n: v: mkGeneralHelper "${n}" "" v;
              in
              flatten (mapAttrsToList mkMap bs);
            mkGeneralTwo =
              bs:
            ...
            mkLsp =
              name: vs:
              optional vs [
                "(${transformName name} . (lambda () (require 'lsp-mode) (lsp-mode)))"
              ];
            mkEglot =
              name: vs:
              optional (vs != false) ''
                :hook (${transformName name} . (lambda () (require 'eglot) (eglot-ensure)))
  ...
  + optionalString hasDiminish ''
    ;; For :diminish in (use-package).
      (require 'diminish)
  ''
  + optionalString cfg.hasOn ''(require 'on)''
  ...
  earlyInitFile = ''
    ...
    ${concatStrings (
      map (p: p.earlyInit) (filter (p: p.earlyInit != "") (builtins.attrValues cfg.usePackage))
    )}
