  ...
  config.programs.emacs.init.usePackage = lib.mkIf keybinds.avy.enable {
    avy = {
      ...
      bindLocal = lib.mkIf (!keybinds.evil.enable) {
        goto-map = {
          "f" = lib.mkIf (!completions.smallExtras.enable) (lib.mkDefault "avy-goto-line");
      ...
      config = ''
                ...
                ${
                  if completions.smallExtras.embark then
                    ''
      ...
      '';
      custom.avy-dispatch-alist = lib.mkIf (!keybinds.evil.enable) (
        lib.mkDefault ''
      ...
      setopt.avy-keys = lib.mkDefault (lib.map (key: "'?${key}") keybinds.avy.avyKeys);
    };
    ...
    evil-easymotion = lib.mkIf keybinds.evil.enable {
      ...
      generalOne = {
        ...
        ":nvo" = {
          ...
          "${keybinds.avy.evilModifierKey}-${keybinds.evil.keys.down}" =
            lib.mkDefault "'evilem-motion-next-visual-line";
          "${keybinds.avy.evilModifierKey}-${keybinds.evil.keys.up}" =
            lib.mkDefault "'evilem-motion-previous-visual-line";
        };
      };
